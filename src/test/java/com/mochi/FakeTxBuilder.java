...
public class FakeTxBuilder {
     ...
     * outputs, one to us, one to somewhere else to simulate change. There is one
     * random input.
     */
    public static Transaction createFakeTxWithChangeAddress(NetworkParameters params, Coin value, Address to,
        ...
        t.addOutput(change);
        // Make a previous tx simply to send us sufficient coins. This prev tx is not
        // really valid but it doesn't
        ...
        // Fake signature.
        // Serialize/deserialize to ensure internal state is stripped, as if it had been
        // read from the wire.
        return roundTripTransaction(params, t);
    }

    /**
     * Create a fake TX for unit tests, for use with unit tests that need greater
     * control. One outputs, 2 random inputs, split randomly to create randomness.
     */
    public static Transaction createFakeTxWithoutChangeAddress(NetworkParameters params, Coin value, Address to) {
        ...
        // Make a random split in the output value so we get a distinct hash when we
        // call this multiple times with same args
    ...
    public static Transaction[] createFakeTx(NetworkParameters params, Coin value, Address to, Address from) {
        // Create fake TXes of sufficient realism to exercise the unit tests. This
        // transaction send BTC from the
        // from address, to the to address with to one to somewhere else to simulate
        // change.
        Transaction t = new Transaction(params);
        TransactionOutput outputToMe = new TransactionOutput(params, t, value, to);
        t.addOutput(outputToMe);
        TransactionOutput change = new TransactionOutput(params, t, valueOf(1, 11),
                LegacyAddress.fromKey(params, new ECKey()));
        t.addOutput(change);
        // Make a feeder tx that sends to the from address specified. This feeder tx is
        // not really valid but it doesn't
        // matter for our purposes.
    ...
    /**
     * Roundtrip a transaction so that it appears as if it has just come from the
     * wire
     */
     ...
     * "to". t2 spends somewhere else. The fake output goes to the same address as
     * t2.
     */
